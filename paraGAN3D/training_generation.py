...
import paraGAN3D.models as models
...
def train(opt):
    ...
    fixed_noise = []
    noise_amp = []
    s = calculateLayerList(opt.train_stages)
    for scale_num in range(opt.stop_scale+1):
        ...
        # myglobal.num_layer_list.append(3)
        d_curr = init_D(opt)
    ...
    if depth == 0:
        ...
        if opt.train_mode == 'generation':
            z_opt = functions.generate_noise([opt.nfc,
                                      ...
                                      reals_shapes[depth][4]+myglobal.num_layer_list[depth]*2],
                                      device=opt.device)
        else:
