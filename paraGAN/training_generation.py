...
import paraGAN.functions as functions
import paraGAN.models as models
...
def train(opt):
    ...
    fixed_condition = []
    noise_amp = []
    s = calculateLayerList(len(reals))
    for scale_num in range(len(reals)):
        opt.out_ = functions.generate_dir2save(opt)
        opt.outf = '%s/%d' % (opt.out_,scale_num)
        # if scale_num >=len(reals)-2:
        #     opt.niter= opt.niter-500
        try:
        ...
        myglobal.currentdepth = s[scale_num]
        myglobal.num_layer_list.append(s[scale_num])
        # myglobal.num_layer_list.append(1)
        d_curr = init_D(opt)
        if scale_num > 0:
            gg=torch.load('%s/%d/netD.pth' % (opt.out_,scale_num-1))
            d_curr.load_state_dict(gg,strict=False)
            generator.init_next_stage(scale_num)
...
def train_single_scale(netD, netG, reals, fixed_noise, noise_amp, opt, depth, writer):
    ...
    if depth == 0:
        ...
        if opt.train_mode == "generation":
            ...
            #                                   device=opt.device)
            if depth < myglobal.WhenEspcn:
                z_opt = functions.generate_noise([opt.nfc,
                                              reals_shapes[depth][2]+myglobal.num_layer_list[depth]*2,
                                              reals_shapes[depth][3]+myglobal.num_layer_list[depth]*2],
                                             device=opt.device)
            else:
                z_opt = functions.generate_noise([opt.nfc,
                                                  reals_shapes[depth-1][2]+6,
                                                  reals_shapes[depth-1][3]+6],
                                                 device=opt.device)
        else:
    ...
    # for block in netG.con[:-opt.train_depth]:
    #     for param in block.parameters():
    #         param.requires_grad = False
    ...
    if depth == 0:
        ...
        noise_amp.append(0)
        z_reconstruction = netG(fixed_noise, reals, noise_amp)

        criterion = nn.MSELoss()
        rec_loss = criterion(z_reconstruction, real)#debug
    ...
    file1 = open('{}/disc_stage{}_loss.txt'.format(opt.out_, depth), 'w+')
    file2 = open('{}/gen_stage{}_loss.txt'.format(opt.out_,depth), 'w+')
    for iter in _iter:
        ...
        for j in range(opt.Dsteps):
            ...
            # train with fake
            # if j == opt.Dsteps - 1:
            #     fake = netG(noise, reals, noise_amp)
            # else:
            #     with torch.no_grad():
            #         fake = netG(noise, reals, noise_amp)
            # with torch.no_grad():
            ...
            errD_total.backward()
            optimizerD.step()
        file1.write(str(functions.move_to_cpu(errD_total))+'\n')
        ############################
        ...
        if alpha != 0:
            loss = nn.MSELoss()
            rec = netG(fixed_noise, reals, noise_amp)
        ...
        errG_total.backward()
        file2.write(str(functions.move_to_cpu(errG_total))+'\n')
...
def generate_samples(netG, opt, depth, noise_amp, writer, reals, iter, n=25):
    ...
    with torch.no_grad():
        for idx in range(n):
            ...
            sample = netG(noise, reals, noise_amp)
            all_images.append(sample)
            functions.save_image(
                '{}/gen_sample_{}.jpg'.format(dir2save, idx), sample.detach())
