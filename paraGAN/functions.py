...
from paraGAN.imresize import imresize, imresize_in, imresize_to_shape
import cv2
...
def generate_condition(img,shape,num):
    img = upsampling(img,shape[0],shape[1])
    condition  = torch.full([img.shape[0],img.shape[1], img.shape[2], img.shape[3]], dtype=torch.float,fill_value=0.,device='cuda')

    a = [i for i in range(25)]
    for i in range(condition.shape[0]):
        lx = 0
        split_size = 5
        while lx + split_size <= condition.shape[2]:
            ly = 0
            while ly + split_size <= condition.shape[3]:
                # random_index = np.random.randint(25, size=25)
                random_index = random.sample(a, 25)
                for r in range(num):
                    condition[i, :, lx + random_index[r] // 5, ly + random_index[r] % 5] = img[i, :,
                                                                                       lx + random_index[r] // 5,
                                                                                       ly + random_index[r] % 5]
                ly = ly + split_size
            lx = lx + split_size
...
def colormap():
    # return matcolor.LinearSegmentedColormap.from_list('cmap', ['#cf0000','#ff4f00','#ffaf00','#efff1f', '#3fffcf', '#0fffff', '#007fff','#0000bf', '#00008f'], 256)
    return matcolor.LinearSegmentedColormap.from_list('cmap',
                                                      [
                                                        '#00008f', '#0000bf','#007fff', '#0fffff', '#3fffcf','#efff1f', '#ffaf00', '#ffaf00', '#ffaf00','#ff4f00','#cf0000', ], 256)

def save_image(name, image):
    image = convert_image_np(image)
    image = image[:,:,1]
    # plt.imsave(name, image, vmin=0, vmax=1, cmap=colormap())
    plt.imsave(name, image, vmin=0, vmax=1)
    ...
    for d in range(depth + 1):
        if d == 0:
            ...
            if opt.train_mode == "generation":
                ...
                #                              device=opt.device).detach())
                if d < myglobal.WhenEspcn:
                    noise.append(generate_noise([opt.nfc,
                                                  reals_shapes[d][2]+ myglobal.num_layer_list[d]*2 ,
                                                  reals_shapes[d][3]+myglobal.num_layer_list[d]*2],
                                                 device=opt.device).detach())
                else:
                    noise.append(generate_noise([opt.nfc,
                                                 reals_shapes[d-1][2] +6,
                                                 reals_shapes[d-1][3]+6],
                                                device=opt.device).detach())
            else:
...
def sample_condition(depth, reals, reals_shapes, opt):
    condition = []
    for d in range(depth + 1):
        if d == 0:
            condition.append(generate_condition(reals[depth],[reals_shapes[d][2], reals_shapes[d][3]],depth+5).detach())
        else:
            if opt.train_mode == "generation":
                # noise.append(generate_noise([opt.nfc, reals_shapes[d][2] + opt.num_layer * 2,
                #                              reals_shapes[d][3] + opt.num_layer * 2],
                #                              device=opt.device).detach())
                if d < myglobal.WhenEspcn:
                    condition.append(generate_condition(reals[depth], [
                                                  reals_shapes[d][2]+ myglobal.num_layer_list[d]*2 ,
                                                  reals_shapes[d][3]+myglobal.num_layer_list[d]*2],depth+5
                                                 ).detach())
                else:
                    condition.append(generate_condition([opt.nfc,
                                                 reals_shapes[d-1][2] +6,
                                                 reals_shapes[d-1][3]+6],depth+5
                                                ).detach())
            else:
                condition.append(generate_noise([opt.nfc, reals_shapes[d][2], reals_shapes[d][3]],
                                             device=opt.device).detach())
    return condition
...
def create_reals_pyramid(real, opt):
    ...
    # use new rescaling method for all other tasks
        ...
        for i in range(opt.stop_scale):
            ...
            reals.append(curr_real)
        reals.append(real)
        # x = [25,35,60,80,120,160,200]
        # for i in x:
        #     curr_real = imresize(real, i / real.shape[2], opt)
        #     reals.append(curr_real)
        # reals.append(real)
        ...
        # reals.append(real)
        # index = 0
        # while a/2>opt.min_size and index<2:
        #     if a%2!=0:
        #         a = a-1
        #         myglobal.updownload[int(a)] = 1
        #     scale_factor = (a/2)/real.shape[2]
        #     curr_real = imresize(real,scale_factor,opt)
        #     a = int(a / 2)
        #     reals.append(curr_real)
        #     index = index +1
        # x = [opt.min_size+25,opt.min_size+8,opt.min_size]
        # for i in x:
        #     curr_real = imresize(real,i/real.shape[2],opt)
        #     reals.append(curr_real)
        # reals.reverse()
...
def generate_dir2save(opt):
    training_image_name1 = opt.input_name[:-4].split("/")[-2]
    training_image_name2 = opt.input_name[:-4].split("/")[-1]
    dir2save = 'TrainedModels/{}/{}/'.format(training_image_name1,training_image_name2)
