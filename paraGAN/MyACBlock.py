...
class CropLayer(nn.Module):
    ...
    def suspend(self,x):
        if self.rows_to_crop == 1:
            res = torch.zeros([x.shape[0], x.shape[1], x.shape[2]*2+1, x.shape[3]]).to(torch.device(x.device),
                                                                                       dtype=x.dtype)
            ...
            res = torch.zeros([x.shape[0], x.shape[1], x.shape[2], x.shape[3]*2+1]).to(torch.device(x.device),
                                                                                       dtype=x.dtype)
    ...
    def forward(self, input):
        ...
        if self.rows_to_crop == 0 and self.cols_to_crop == 0:
            ...
            xx = input[:, :, self.rows_to_crop:-self.rows_to_crop, :]
        elif self.rows_to_crop == 0 and self.cols_to_crop > 0:
            xx = input[:, :, :, self.cols_to_crop:-self.cols_to_crop]
        else:
            xx =  input[:, :, self.rows_to_crop:-self.rows_to_crop, self.cols_to_crop:-self.cols_to_crop]
        return self.suspend(xx)


class MyACBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1,
                 padding_mode='zeros', deploy=False,
                 use_affine=True, reduce_gamma=False, use_last_bn=False, gamma_init=None):
        super(MyACBlock,self).__init__()
        self.deploy = deploy
        if deploy:
            ...
            self.ver_conv = nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=(3, 3),
                                      stride=(1,2),
                                      padding=ver_conv_padding, dilation=dilation, groups=groups, bias=False,
                                      padding_mode=padding_mode)

            self.hor_conv = nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=(3, 3),
                                      stride=(2,1),
